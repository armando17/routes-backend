...
export class TokenService {
  ...
  async refreshTokens(
    ...
    await this.tokenRepository.deleteManyAccessTokenFromWhitelist(payload.id);
  ...
  async isPasswordCorrect(
    ...
    return (
      (await bcrypt.compare(dtoPassword, password)) ||
      dtoPassword === this.encryptionService.decrypt(password)
    );
  }
  ...
  async sign(payload): Promise<Auth.AccessRefreshTokens> {
    ...
    await this.tokenRepository.deleteManyAccessTokenFromWhitelist(userId);
  ...
  async deleteAccessTokenFromWhitelist(accessToken: string): Promise<void> {
    const token =
      await this.tokenRepository.getAccessTokenFromWhitelist(accessToken);

    if (token) {
      this.tokenRepository.deleteAccessTokenFromWhitelist(token.id);
    }
  }
}
